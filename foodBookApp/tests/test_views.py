...
class AuthenticationTest(TestCase):
    def test_register(self):
        response = self.client.post('/register/', {'username':'test_user1','email':'test_user1@email.com','password1':'a_super_duper_key', 'password2':'a_super_duper_key'})
        self.assertRedirects(response, '/login/')
        self.assertTrue(User.objects.filter(username='test_user1'))
        self.assertFalse(response.context)

        response = self.client.post('/register/', {'username':'bad','email':'bad','password1':'bad', 'password2':'a'})
...
class SearchTest(TestCase):
    ...
    def search_success_results_count(self, query, context, count):
        response = self.client.get('/search/?q={}'.format(query))
        self.assertEqual(response.status_code, 200)
    ...
    def test_no_results(self):
        self.search_success_results_count('abc', 'profiles', 0)
        self.search_success_results_count('abc', 'posts', 0)

    def test_profile_search(self):
        user2 = User.objects.get(pk=2)
        profile2 = Profile.objects.get(user=user2)
        profile2.privacy = 'private'
        self.search_success_results_count('user2', 'profiles', 1)
        profile2.privacy = 'friends'
        self.search_success_results_count('user2', 'profiles', 1)
        self.search_success_results_count('test', 'profiles', 2)
    ...
    def test_post_search(self):
        ...
        self.search_success_results_count('abc', 'posts', 1)
        # post1.privacy = 'private'
        # self.assertTrue(post1.privacy == 'private')
        # self.assertFalse(post1.privacy == 'public')
        # self.search_success_results_count('abc', 'posts', 0)
